...
public class Ticket {
    ...
    public static void atualizarTicket() {
        ...
        while (true) {
            idTicket = Integer.parseInt(JOptionPane.showInputDialog(null, "Insira o id do seu ticket", "Atualizar ticket", JOptionPane.QUESTION_MESSAGE));

            try (Connection conn = Conexao.conectar()) {
                String sql = "SELECT * FROM tb_tickets WHERE id_ticket = ? AND id_usuario = ?";
